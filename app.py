...
# Optional: Turtle graphics (local desktop only; not on Streamlit Cloud)
try:
    import turtle
...
def adherence_this_week(today: dt.date) -> int:
    ...
    if total == 0:
        return 0
    taken = 0
    for occ_list in week_occ.values():
        for ev in occ_list:
...
with st.sidebar:
    ...
    if st.button("Test beep"):
        maybe_beep()
    st.caption("Keep the app open to see and hear reminders when doses are near.")
...
st.write("Add recurring schedules, see today's checklist, view a weekly calendar, and celebrate adherence. Designed with large fonts and soft colors.")

col_input, col_main, col_side = st.columns([1.3, 1.7, 1.2])


# ------------------------------
# Input: add schedules (hybrid input)
# ------------------------------
with col_input:
    ...
    # Pre-select today's weekday so new schedules show in today's checklist
    today = dt.date.today()
    today_wname = weekday_name(today)
    default_days = [today_wname]

    days_selected = st.multiselect("Days of week", DAY_ORDER, default=default_days)

    # Multiple doses per selected day
    dose_count = st.number_input("Doses per day", min_value=1, max_value=6, value=1, step=1)
    dose_times = []
    for i in range(dose_count):
        dose_times.append(st.time_input(f"Dose time #{i+1}", value=dt.time(9 + i*3, 0), key=f"dose_time_{i}"))
    ...
    if st.button("Add schedule"):
        ...
        if not final_name:
            ...
            st.warning("Please select at least one day of the week.")
        else:
            ...
            st.success(f"Added schedule for {final_name} on {', '.join(days_selected)} at {[t.strftime('%H:%M') for t in dose_times]}")
            # If today is among selected days and time is within reminder window, beep now
            now_time = dt.datetime.now().time()
            if today_wname in days_selected and today >= start_date:
                for t in dose_times:
                    if now_time < t:
                        delta_min = (dt.datetime.combine(today, t) - dt.datetime.combine(today, now_time)).total_seconds() / 60.0
                        if 0 <= delta_min <= st.session_state.reminder_minutes:
                            maybe_beep()
                            st.toast(f"Reminder: {final_name} due at {t.strftime('%H:%M')} soon.")
...
# ------------------------------
# Main: today’s checklist with color codes & reminders
# ------------------------------
with col_main:
            ...
            with cA:
                if status != "taken":
                    if st.button("Mark taken", key=f"taken_btn_{i}"):
                        ...
                        st.experimental_rerun()
            with cB:
                # Optional skip/reset (simple demo actions)
                if status == "missed":
                    if st.button("Reset missed", key=f"reset_btn_{i}"):
                        # Remove from taken if present (no-op for missed)
                        if key in st.session_state.taken_events:
                            st.session_state.taken_events.remove(key)
                        st.info("Reset done.")
                        st.experimental_rerun()
            with cC:
                if st.button("Play beep", key=f"beep_btn_{i}"):
                    maybe_beep()
...
with col_side:
    ...
    st.markdown("---")
    st.caption("Audio alerts play when a dose is within your reminder window. Turtle drawings open locally.")
